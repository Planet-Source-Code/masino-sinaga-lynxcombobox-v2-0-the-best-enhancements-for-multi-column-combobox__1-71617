VERSION 5.00
Begin VB.UserControl LynxComboBox 
   AutoRedraw      =   -1  'True
   BackColor       =   &H80000005&
   ClientHeight    =   2175
   ClientLeft      =   0
   ...
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   253
   ToolboxBitmap   =   "LynxComboBox.ctx":0000
   ...
   End
End
Attribute VB_Name = "LynxComboBox"
...
'#############################################################################################################################
'Title:     LynxComboBox (previously called ComboView)
'Function:  Owner-drawn multi-column ComboBox with checkbox mode
'Author:    Richard Mewett
'Created:   01/06/05
'Version:   1.3 (11 November 2006)

'Last-mod:  January 6, 2009
'Mod-by:    Masino Sinaga, admin@masinosinaga.com, http://www.masinosinaga.com

'Copyright © 2005-2006 Richard Mewett. All rights reserved.

'This control can be used as an alternative to a ComboBox. It combines
'ComboBox operation with a ListView (Style=Report) functionality

'#############################################################################################################################
'Credits:   Paul Caton - Subclassing
'           Matt Usner - Rounded Region Code
'           Heriberto Mantilla Santamaria - Creating Window from PictureBox
'           Dana Seaman - DrawText wrapper & Unicode Res file for Demo
'           Gary Noble - API Scroll Bars
'           LaVolpe - suggestions for LynxGrid implemented in LynxComboBox
'           Riccardo Cohen - Bug reports & ownerdrawn XP/Office ThemeStyles
'           Marcelo Luiz Altafin - MyControls and MyControl class module
'           Masino Sinaga - Bug fixed for Locked functionality,
'                         - Added FindFirstText, FindNextText, FindPrevText, FindLastText
'                         - Optimized SearchCode function for validate the selected item
'                         - Added new feature for binding another controls to LynxComboBox

'       +   Thankyou to everyone who has reported bugs

'#############################################################################################################################
'This software is provided "as-is," without any express or implied warranty.
'In no event shall the author be held liable for any damages arising from the
'use of this software.
'If you do not agree with these terms, do not install "LynxComboBox". Use of
...
Public Enum ScrollBarlcStyleEnum
...
Private m_eStyle            As ScrollBarlcStyleEnum
...
Private Enum lcFlagsEnum
    lcChecked = 2
    lcSelected = 4
    lcBold = 8
End Enum

'Modified by Masino Sinaga, January 4, 2009; from Private to Public
'in order this enum could be used for FindFirstText, FindNextText,
'FindPrevText, and FindLastText functions...
Public Enum lcSearchEnum
    lcEqual = 0
    lcGreaterEqual = 1
    lcLike = 2
End Enum

Public Enum lcColAlignmentEnum
    lcAlignLeftTop = DT_LEFT Or DT_TOP
    lcAlignLeftCenter = DT_LEFT Or DT_VCENTER
    lcAlignLeftBottom = DT_LEFT Or DT_BOTTOM
    lcAlignCenterTop = DT_CENTER Or DT_TOP
    lcAlignCenterCenter = DT_CENTER Or DT_VCENTER
    lcAlignCenterBottom = DT_CENTER Or DT_BOTTOM
    lcAlignRightTop = DT_RIGHT Or DT_TOP
    lcAlignRightCenter = DT_RIGHT Or DT_VCENTER
    lcAlignRightBottom = DT_RIGHT Or DT_BOTTOM
End Enum

Public Enum lcBorderStyleEnum
    lcBorderNone = 0
    lcBorderSunken = 1
    lcBorderRaised = 2
    lcBorderFlat = 3
...
Public Enum lcFocusRectStyleEnum
    lcFocusRectNone = 0
    lcFocusRectLight = 1
    lcFocusRectHeavy = 2
End Enum

Private Enum lcHeaderStateEnum
    lcNormal = 1
    lcHot = 2
    lcDown = 3
End Enum

Public Enum lcSortTypeEnum
    lcSTAscending = 0
    lcSTDescending = 1
End Enum

Public Enum lcStyleEnum
    lcStandard = 0
    lcCheckboxes = 1
    lcOptionButtons = 2
End Enum

Public Enum lcThemeStyleEnum
    lcTSWindows3D = 0
    lcTSWindowsFlat = 1
    lcTSWindowsXP = 2
    lcTSOfficeXP = 3
...
#If False Then
    Private lcChecked, lcSelected, lcBold
    Private lcEqual, lcGreaterEqual, lcLike
    Private lcAlignLeftTop, lcAlignLeftCenter, lcAlignLeftBottom, lcAlignCenterTop, lcAlignCenterCenter, lcAlignCenterBottom, lcAlignRightTop, lcAlignRightCenter, lcAlignRightBottom
    Private lcBorderNone, lcBorderSunken, lcBorderRaised, lcBorderFlat, lcBorderCustom
    Private lcString, lcNumeric, lcDate, lcCustom
    Private lcFocusRectNone, lcFocusRectLight, lcFocusRectHeavy
    Private lcNormal, lcHot, lcDown
    Private lcSTAscending, lcSTDescending
    Private lcStandard, lcCheckboxes, lcOptionButtons
    Private lcTSWindows3D, lcTSWindowsFlat, lcTSWindowsXP, lcTSOfficeXP
#End If

Private Const BORDER_LEFT       As Long = 4
Private Const BORDER_TOP        As Long = 3
Private Const BUTTON_WIDTH      As Long = 15
Private Const SCROLLBAR_SIZE    As Long = 16

Private Const DEF_ALIGNMENT             As Long = vbLeftJustify
Private Const DEF_ARROWCOLOR            As Long = vbButtonText
Private Const DEF_AUTOCOMPLETE          As Boolean = False
Private Const DEF_BACKCOLOR             As Long = vbWindowBackground
Private Const DEF_BACKCOLOREDIT         As Long = vbWindowBackground
Private Const DEF_BACKCOLORFIXED        As Long = vbButtonFace
Private Const DEF_BACKCOLORSEL          As Long = vbHighlight
Private Const DEF_BORDERCOLOR           As Long = vbBlack
Private Const DEF_BORDERCURVE = 5
Private Const DEF_BORDERSTYLE = 1
Private Const DEF_BORDERWIDTH = 1
Private Const DEF_BUTTONBACKCOLOR       As Long = vbButtonFace
Private Const DEF_CACHE_INCREMENT       As Long = 10
Private Const DEF_COLS = 1
Private Const DEF_COLUMNHEADERS         As Boolean = False
Private Const DEF_COLUMNRESIZE          As Boolean = False
Private Const DEF_COLUMNSORT            As Boolean = False
Private Const DEF_DROPDOWNAUTOWIDTH     As Boolean = False
...
Private Const DEF_INTEGRALHEIGHT        As Boolean = False
Private Const DEF_LOCKED                As Boolean = False
Private Const DEF_PAGESCROLLITEMS = 8
Private Const DEF_REQUIRECHECKEDITEM    As Boolean = False
...
Private Const DEF_TEXTALL               As String = "-- All --"
Private Const DEF_TEXTNONE              As String = "-- None --"
Private Const DEF_TEXTSELECTION         As String = "-- Selection --"
Private Const DEF_THEMESTYLE            As Long = lcThemeStyleEnum.lcTSWindowsXP
...
Private Type ColType
    nAlignment As lcColAlignmentEnum
    dCustomWidth As Single
    lX As Long
    lWidth As Long
    nSortOrder As lcSortTypeEnum
    nType As Integer
    bVisible As Boolean
    sCaption As String
End Type
...
Private mArrowColor As Long
Private mBorderColor As Long
Private mButtonBackColor As Long
Private mFocusRectColor As Long
Private mHotArrowColor As Long
Private mHotBorderColor As Long
Private mHotButtonBackColor As Long

Private mBorderCurve As Long
Private mBorderStyle As lcBorderStyleEnum
Private mBorderWidth As Long
Private mDisplayEllipsis  As Boolean
Private mDropDownFont As Font
Private mFocusRectStyle As lcFocusRectStyleEnum
Private mFont As Font
Private mThemeStyle As lcThemeStyleEnum
...
Private mStyle As lcStyleEnum
Private mTextAll As String
Private mTextNone As String
Private mTextSelection As String

'Added by Masino Sinaga, Januari 4, 2009 to flag the last position record in the drop-down-list
'while searching text by using FindFirstText, FindNextText, FindPrevText, and FindLastText
Private lPos As Long
Dim m_MyControls As New MyControls
...
Public Event CustomSort(Ascending As Boolean, Col As Long, Value1 As String, Value2 As String, bSwap As Boolean)
Public Event DropDownClose()
Public Event DropDownOpen()
Public Event EditTextChanged(ListIndex As Long, OriginalText As String, NewText As String, Cancel As Boolean)
...
'Parameters:
  ...
  'lParam   - Message related data
    ...
    Case WM_MOUSELEAVE
        ...
        If mBorderStyle = lcBorderCustom Then
            DrawComboBorder
        End If
...
Public Sub AddItem(ByVal Item As String, Optional Index As Long = -1, Optional Checked As Boolean, Optional Image As Variant)
    Dim lCol As Long
    Dim lCount As Long
    Dim sText() As String
    
    '#############################################################################################################################
    'Purpose: Add an Item (new Row) to the ComboBox
    
    'Item       - This contains the data for the Cells in the new Row. You can pass multiple
    '           Cells by using a Delimiter between data
    'Index      - Allows a new Item to be Inserted before an existing one
    'Checked    - Default Checked state of the new Item
    'Image      - The Key to an Image in the ImageList
    ...
    'Note MAX_ITEMS is the Max value for a long - in practice you would
    'not want to load over 100K Items
    If mItemCount = MAX_ITEMS Then
        Err.Raise 381, "LynxComboBox", "Maximum ListItems Exceeded"
    Else
        ...
        If UBound(mCols) > 0 Then
            ...
            For lCount = LBound(sText) To UBound(sText)
                mItems(mItemCount).sValue(lCol) = sText(lCount)
                lCol = lCol + 1
        ...
        With mItems(mItemCount)
            ...
            If Checked Then
                SetFlag mItemCount, lcChecked, True
            End If
            
            'Default Bold
            If mDropDownFont.Bold Then
                SetFlag mItemCount, lcBold, True
            End If
        End With
        
        ItemImage(mItemCount) = Image
    End If
End Sub
...
Public Property Let Alignment(ByVal NewValue As AlignmentConstants)
    mAlignment = NewValue
    txtEdit.Alignment = mAlignment
    
    PropertyChanged "Alignment"
End Property

Public Property Get ArrowColor() As OLE_COLOR
Attribute ArrowColor.VB_ProcData.VB_Invoke_Property = ";Appearance"
    ArrowColor = mArrowColor
End Property

Public Property Let ArrowColor(ByVal NewValue As OLE_COLOR)
    mArrowColor = NewValue
    DrawComboBorder
...
Public Property Let BackColorEdit(ByVal lNewValue As OLE_COLOR)
    ...
    txtEdit.BackColor = mBackColorEdit
    DrawEditText mInFocus
...
Public Property Get BorderStyle() As lcBorderStyleEnum
Attribute BorderStyle.VB_ProcData.VB_Invoke_Property = ";Appearance"
    BorderStyle = mBorderStyle
End Property

Public Property Let BorderStyle(ByVal NewValue As lcBorderStyleEnum)
...
Public Property Get ColAlignment(ByVal Index As Long) As lcColAlignmentEnum
    ColAlignment = mCols(Index).nAlignment
End Property

Public Property Let ColAlignment(ByVal Index As Long, ByVal NewValue As lcColAlignmentEnum)
...
Public Property Get ColType(ByVal Index As Long) As lcColTypeEnum
    ColType = mCols(Index).nType
End Property

Public Property Let ColType(ByVal Index As Long, ByVal NewValue As lcColTypeEnum)
...
Private Sub CreateRenderData()
    ...
    With mR
        ...
        ElseIf lSize = 0 Then
            .CheckBoxSize = picList.TextHeight("A")
        Else
            .CheckBoxSize = lSize - 4
        End If

        If mStyle <> lcStandard Then
        ...
        If mImageList Is Nothing Then
            .ImageSpace = 0
        Else
            .ImageSpace = ((mR.TextHeight - mImageList.ImageHeight) / 2)
...
Private Sub DrawArrow(hdc As Long, lX As Long, lY As Long, lWidth As Long, lStep As Long, lcolor As Long, bUpArrow As Boolean)
    '#############################################################################################################################
    ...
    hPen = CreatePen(0, 1, TranslateColor(lcolor))
...
Private Sub DrawComboBorder()
    ...
    With mButtonRect
        .Left = txtEdit.Width + BORDER_LEFT + 1
        .Top = BORDER_TOP - 1
    ...
    With UserControl
        ...
        If mBorderStyle = lcBorderCustom Then
            Call SetRect(R, txtEdit.Width + BORDER_LEFT + 1, 0, .ScaleWidth, .ScaleHeight)
            If mInCtrl Then
        ...
        If mEnabled Then
            If mInCtrl Then
                lcolor = mHotArrowColor
            Else
                lcolor = mArrowColor
            End If
        Else
            lcolor = vbButtonShadow
        End If
        
        DrawArrow .hdc, lX, lY, 6, 3, lcolor, False
...
Private Sub DrawEditText(Optional bFocus As Boolean)
    Dim R As RECT
    ...
    With UserControl
        ...
        If mEditable Then
            ...
            End If
        Else
            If (mBorderStyle = lcBorderCustom) And (mBorderCurve > 0) Then
                Call SetRect(R, BORDER_LEFT + 2, BORDER_TOP, BORDER_LEFT + txtEdit.Width, BORDER_TOP + txtEdit.Height)
            Else
                Call SetRect(R, BORDER_LEFT - 1, BORDER_TOP, BORDER_LEFT + txtEdit.Width, BORDER_TOP + txtEdit.Height)
            End If
            ...
            If mEnabled Then
                If bFocus Then
                    ...
                    DrawRect .hdc, R, TranslateColor(vbHighlight), True
                    
                    .ForeColor = vbWindowText
                    Call DrawFocusRect(.hdc, R)
                    .ForeColor = vbHighlightText
                Else
                    DrawRect .hdc, R, TranslateColor(mBackColorEdit), True
...
Private Sub DrawHeader(lCol As Long, State As lcHeaderStateEnum)
    '#############################################################################################################################
    ...
    If lCol > NULL_RESULT Then
        With picList
            ...
            DrawRect .hdc, R, TranslateColor(mBackColorFixed), True
            ...
            If mCols(lCol).lWidth > SIZE_SORTARROW Then
                If lCol = mSortColumn Then
                    DrawArrow .hdc, (mCols(lCol).lX + mCols(lCol).lWidth) - 12, 6, 9, 5, TranslateColor(vbButtonShadow), (mCols(lCol).nSortOrder = lcSTDescending)
                ...
                ElseIf lCol = mSortSubColumn Then
                    DrawArrow .hdc, (mCols(lCol).lX + mCols(lCol).lWidth) - 12, 6, 6, 3, TranslateColor(vbButtonShadow), (mCols(lCol).nSortOrder = lcSTDescending)
...
Private Sub DrawList()
    ...
    With picList
        ...
        '#############################################################################################################################
        'List Items
        For lRow = SBValue(efsVertical) To (SBValue(efsVertical) + mDropDownItemsVisible) - 1
            If lRow > mItemCount Then
                Exit For
            End If
            
            If lRow = mHighlighted Then
                'Draw Highlight & Focus Rectangles
                Call SetRect(R, mR.LeftText, lY, .ScaleWidth, lY + mR.TextHeight)
                DrawRect .hdc, R, TranslateColor(mBackColorSel), True
                
                Select Case mFocusRectStyle
                    Case lcFocusRectLight
                        Call DrawFocusRect(.hdc, R)
                    Case lcFocusRectHeavy
                        DrawRect .hdc, R, TranslateColor(mFocusRectColor), False
                End Select
                
                .ForeColor = mForeColorSel
            Else
                .ForeColor = mItems(mRowPtr(lRow)).lForeColor
            End If
            .FontBold = mItems(mRowPtr(lRow)).nFlags And lcBold
            
            Select Case mStyle
                Case lcCheckboxes
                    Call SetRect(R, 3, lY, mR.CheckBoxSize, lY + mR.TextHeight)
                                
                    If mItems(mRowPtr(lRow)).nFlags And lcChecked Then
                        lValue = 5
                    Else
                        lValue = 0
                    End If
                ...
                Case lcOptionButtons
                    ...
                    If Not DrawTheme("Button", 3, lValue, R) Then
                        If mItems(mRowPtr(lRow)).nFlags And lcChecked Then
                            Call DrawFrameControl(.hdc, R, DFC_BUTTON, DFCS_BUTTONRADIO Or DFCS_CHECKED Or DFCS_FLAT)
                        Else
                            Call DrawFrameControl(.hdc, R, DFC_BUTTON, DFCS_BUTTONRADIO Or DFCS_FLAT)
                        End If
                    End If
            End Select
            ...
            If bRenderImages Then
                If mR.ImageSpace > 0 Then
                    ...
                    If mItems(mRowPtr(lRow)).lImage <> 0 Then
                        ...
                        If lRow = mHighlighted Then
                            mImageList.ListImages(Abs(mItems(mRowPtr(lRow)).lImage)).Draw .hdc, lImageLeft, ScaleY(lY + mR.ImageSpace, vbPixels, nImageListScaleMode), 2
                        Else
                            mImageList.ListImages(Abs(mItems(mRowPtr(lRow)).lImage)).Draw .hdc, lImageLeft, ScaleY(lY + mR.ImageSpace, vbPixels, nImageListScaleMode), 1
                        End If
                    End If
                End If
            End If
            ...
            For lCol = lStartCol To UBound(mCols)
                If mCols(lCol).bVisible Then
                    If lX < 0 Then
                        lX = 1
                        Call SetRect(R, mR.LeftText, lY, (mR.LeftText + mCols(lCol).lWidth) - mR.LeftText, lY + mR.TextHeight)
                     Else
                        Call SetRect(R, lX, lY, (lX + mCols(lCol).lWidth) - 3, lY + mR.TextHeight)
                     End If
                
                    Call DrawText(.hdc, mItems(mRowPtr(lRow)).sValue(lCol), -1, R, mCols(lCol).nAlignment Or mR.DTFlag)
    ...
    'Debug.Print "DrawList " & Timer
End Sub

Private Sub DrawOfficeXPHeader(lhDC As Long, rRect As RECT, State As lcHeaderStateEnum)
    '#############################################################################################################################
...
Private Function DrawTheme(sClass As String, ByVal iPart As Long, ByVal iState As Long, rtRect As RECT, Optional ByVal CloseTheme As Boolean = False) As Boolean
    ...
    If mWindowsXP Then
        If (mThemeStyle = lcTSWindowsXP) Or (mThemeStyle = lcTSOfficeXP) Then
            hTheme = OpenThemeData(UserControl.hWnd, StrPtr(sClass))
            If (hTheme) Then
                lResult = DrawThemeBackground(hTheme, picList.hdc, iPart, iState, rtRect, rtRect)
                DrawTheme = (lResult = 0)
            Else
                DrawTheme = False
            End If
...
Private Sub DrawXPHeader(lhDC As Long, rRect As RECT, State As lcHeaderStateEnum)
    '#############################################################################################################################
...
Attribute DropDownAutoWidth.VB_ProcData.VB_Invoke_Property = ";Behavior"
...
Attribute DropDownFont.VB_ProcData.VB_Invoke_Property = ";Appearance"
...
Attribute DropDownItemsVisible.VB_ProcData.VB_Invoke_Property = ";Appearance"
...
Attribute DropDownWidth.VB_ProcData.VB_Invoke_Property = ";Appearance"
...
Public Property Get FocusRectStyle() As lcFocusRectStyleEnum
Attribute FocusRectStyle.VB_ProcData.VB_Invoke_Property = ";Appearance"
    FocusRectStyle = mFocusRectStyle
End Property

Public Property Let FocusRectStyle(ByVal NewValue As lcFocusRectStyleEnum)
...
Public Property Let ForeColorEdit(ByVal lNewValue As OLE_COLOR)
    ...
    txtEdit.ForeColor = mForeColorEdit
    DrawEditText mInFocus
...
Public Property Let FormatString(ByVal NewValue As String)
    ...
    For lCol = LBound(sCols) To UBound(sCols)
        ...
        Case "^"
            ...
            mCols(lCol).nAlignment = lcAlignCenterTop
        Case "<"
            mCols(lCol).sCaption = Mid$(sCols(lCol), 2)
            mCols(lCol).nAlignment = lcAlignLeftTop
        Case ">"
            mCols(lCol).sCaption = Mid$(sCols(lCol), 2)
            mCols(lCol).nAlignment = lcAlignRightTop
        Case Else
...
Private Function GetFlag(ByVal Index As Long, nFlag As lcFlagsEnum) As Boolean
    ...
    With picList
        If mColumnHeaders Then
            ...
            If Y > lColumnHeadingHeight Then
                lRow = ((Y - lColumnHeadingHeight) \ mR.TextHeight) + SBValue(efsVertical)
            Else
                lRow = -1
            End If
        Else
            lRow = (Y \ mR.TextHeight) + SBValue(efsVertical)
        End If
    End With
...
Public Property Get HotArrowColor() As OLE_COLOR
Attribute HotArrowColor.VB_ProcData.VB_Invoke_Property = ";Appearance"
    HotArrowColor = mHotArrowColor
End Property

Public Property Let HotArrowColor(ByVal NewValue As OLE_COLOR)
    mHotArrowColor = NewValue
    DrawComboBorder
    
    PropertyChanged "HotArrowColor"
End Property
...
Public Property Get ItemChecked(ByVal Index As Long) As Boolean
    ItemChecked = GetFlag(mRowPtr(Index), lcChecked)
End Property

Public Property Let ItemChecked(ByVal Index As Long, ByVal NewValue As Boolean)
    SetFlag mRowPtr(Index), lcChecked, NewValue
End Property
...
'Modified by Masino Sinaga; January 4, 2009; Index become Row; Item become Column ... for easier to understand
Public Property Get ItemText(ByVal Row As Long, ByVal Column As Long) As String
    If UBound(mItems(mRowPtr(Row)).sValue) >= Column Then
        ItemText = mItems(mRowPtr(Row)).sValue(Column)
    End If
End Property
'Modified by Masino Sinaga; January 4, 2009; Index become Row; Item become Column ... for easier to understand
Public Property Let ItemText(ByVal Row As Long, ByVal Column As Long, NewValue As String)
    If UBound(mItems(mRowPtr(Row)).sValue) >= Column Then
        mItems(mRowPtr(Row)).sValue(Column) = NewValue
    End If
End Property

Public Property Get List(ByVal Index As Long) As String
    List = mItems(mRowPtr(Index)).sValue(0)
End Property
...
Attribute PageScrollItems.VB_ProcData.VB_Invoke_Property = ";Behavior"
...
Private Sub picList_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If (Button = vbLeftButton) And Not mLocked Then
        ...
        'MsgBox mMouseDownRow & "," & lCurrentMouseCol
        
        If Y < mR.HeaderHeight Then
            If (picList.MousePointer <> vbSizeWE) Then
                mMouseDownCol = lCurrentMouseCol
                If mMouseDownCol <> NULL_RESULT Then
                    With picList
                        DrawHeader mMouseCol, lcDown
                        .Refresh
                    End With
                End If
            End If
        ElseIf mMouseDownRow >= 0 Then
            mListIndex = mMouseDownRow
            RaiseEvent ClickItem(mMouseDownRow, Button, Shift)
        
            Select Case mStyle
                Case lcCheckboxes
                    bValue = Not GetFlag(mRowPtr(mListIndex), lcChecked)
                    RaiseEvent RequestItemChecked(mRowPtr(mListIndex), bValue, bCancel)
                Case lcOptionButtons
                    bValue = True
                    RaiseEvent RequestItemChecked(mRowPtr(mListIndex), bValue, bCancel)
            End Select
            ...
            If Not bCancel Then
                SetFlag mRowPtr(mListIndex), lcChecked, bValue
                DrawList
                UpdateEditText mListIndex, False
...
Private Sub picList_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If Not mLocked Then
        ...
        If mColumnHeaders Then
            ...
            If mMouseDownCol <> NULL_RESULT Then
                If (mMouseDownCol = mMouseCol) And (mMouseRow = NULL_RESULT) Then
                    DrawHeader mMouseCol, lcDown
                Else
                    DrawHeader mMouseDownCol, lcNormal
                End If
                picList.Refresh
            End If
        ...
        ElseIf Button = 0 Then
            ...
            If (mMouseRow >= 0) Then
                If (mHighlighted <> mMouseRow) Then
                    mHighlighted = mMouseRow
                    DrawList
                End If
            ElseIf mColumnResize Then
                 For mMouseRow = LBound(mCols) To UBound(mCols)
                    lWidth = lWidth + mCols(mMouseRow).lWidth
                    
                    If (X < lWidth + 2) And (X > lWidth - 2) Then
                        nPointer = vbSizeWE
                        mResizeCol = mMouseRow
                        Exit For
                    End If
                    
                    lResizeX = lResizeX + mCols(mMouseRow).lWidth
                Next mMouseRow
            End If
                ...
                If mColumnSort Then
                    If (Shift And vbCtrlMask) And (mSortColumn <> NULL_RESULT) Then
                        If mSortSubColumn <> mMouseDownCol Then
                            mCols(mMouseDownCol).nSortOrder = lcSTAscending
                        End If
                    ...
                    Else
                        ...
                        End If
                    End If
                    'MsgBox "A: " & mMouseDownCol
                Else
                    DrawHeaderRow
                    'MsgBox "B: " & mMouseDownCol
                    RaiseEvent ColumnClick(mMouseDownCol)
                End If
        
            ElseIf mMouseDownCol >= 0 Then
                DrawList
            End If
        ElseIf (mStyle = lcStandard) And (mResizeCol < 0) Then
            mListIndex = mMouseRow
            
            SetDropDown
        End If
    End If
    'MsgBox mMouseDownCol
    mMouseDown = False
    mMouseDownCol = NULL_RESULT
    mResizeCol = NULL_RESULT
End Sub
...
Public Sub Refresh()
    ...
    UpdateEditText mListIndex, mInFocus
...
Public Sub RemoveItem(ByVal Index As Long)
    ...
    bSelected = mItems(mRowPtr(Index)).nFlags And lcSelected
    ...
    Else
        ...
        If bSelected Then
            If mStyle = lcCheckboxes Then
                RaiseEvent SelectionChanged
            ElseIf Index > mItemCount Then
                SetFlag mItems(mRowPtr(mItemCount)).nFlags, lcSelected, True
            ElseIf mItemCount >= 0 Then
                SetFlag mItems(mRowPtr(Index)).nFlags, lcSelected, True
            End If
        End If
        
        If mListIndex > mItemCount Then
            mListIndex = mItemCount
        End If
    End If
End Sub
...
Private Property Get SBStyle() As ScrollBarlcStyleEnum
    SBStyle = m_eStyle
End Property

Private Property Let SBStyle(ByVal eStyle As ScrollBarlcStyleEnum)
    Dim lR As Long
    If (m_bNoFlatScrollBars) Then
        ' can't do it..
...
'Modified by Masino Sinaga, January 4, 2009; change from Private to Public
'in order this function could be used for validating selected item by user
Public Function SearchCode(sCode As String, nMode As lcSearchEnum) As Long
    Dim lCount As Long
    SearchCode = NULL_RESULT
    For lCount = LBound(mItems) To mItemCount
        Select Case nMode
        Case lcEqual
            If UCase$(mItems(mRowPtr(lCount)).sValue(mSearchColumn)) = sCode Then
                SearchCode = lCount
                Exit For
            End If
        Case lcGreaterEqual
            If UCase$(Left$(mItems(mRowPtr(lCount)).sValue(mSearchColumn), Len(sCode))) >= sCode Then
                SearchCode = lCount
                Exit For
            End If
        Case lcLike
            If UCase$(mItems(mRowPtr(lCount)).sValue(mSearchColumn)) Like "*" & sCode & "*" Then
                SearchCode = lCount
                Exit For
            End If
        End Select
...
'Added by Masino Sinaga (admin@masinosinaga.com), January 4, 2009
'This function will search for the text at the first item/record found in the drop-down list
'based on the certain text; include the search mode; and in which column the text will
'be searched (this function was created based on SearchCode function above)
Public Function FindFirstText(sText As String, nMode As lcSearchEnum, SearchColumn As Integer) As Long
    Dim lCount As Long
    FindFirstText = NULL_RESULT
    For lCount = LBound(mItems) To mItemCount
        Select Case nMode
        Case lcEqual
          If ItemText(lCount, SearchColumn) = sText Then
            FindFirstText = lCount
            lPos = lCount
            Exit For
          End If
        Case lcGreaterEqual
          If Left$(ItemText(lCount, SearchColumn), Len(sText)) >= sText Then
            FindFirstText = lCount
            lPos = lCount
            Exit For
          End If
        Case lcLike
          If UCase$(ItemText(lCount, SearchColumn)) Like "*" & UCase$(sText) & "*" Then
            FindFirstText = lCount
            lPos = lCount
            Exit For
          End If
        End Select
    Next lCount
    Text = ItemText(lPos, 0)
End Function

'Added by Masino Sinaga (admin@masinosinaga.com), January 4, 2009
'This function will search for the text at the next item/record found in the drop-down list
'based on the certain text; include the search mode; and in which column the text will
'be searched (this function was created based on SearchCode function above)
Public Function FindNextText(sText As String, nMode As lcSearchEnum, SearchColumn As Integer) As Long
    Dim lCount As Long
    For lCount = lPos + 1 To mItemCount
        Select Case nMode
        Case lcEqual
          If ItemText(lCount, SearchColumn) = sText Then
            FindNextText = lCount
...
'This function will search for the text at the previouse item/record found in the drop-down list
'based on the certain text; include the search mode; and in which column the text will
'be searched (this function was created based on SearchCode function above)
Public Function FindPrevText(sText As String, nMode As lcSearchEnum, SearchColumn As Integer) As Long
    Dim lCount As Long
    For lCount = lPos - 1 To 1 Step -1 'mItemCount To lPos - 1 Step -1
        Select Case nMode
        Case lcEqual
          If ItemText(lCount, SearchColumn) = sText Then
            FindPrevText = lCount
...
'Added by Masino Sinaga (admin@masinosinaga.com), January 6, 2009
'This function will search for the text at the last item/record found in the drop-down list
'based on the certain text; include the search mode; and in which column the text will
'be searched (this function was created based on SearchCode function above)
Public Function FindLastText(sText As String, nMode As lcSearchEnum, SearchColumn As Integer) As Long
    Dim lCount As Long
    For lCount = lPos + 1 To mItemCount
        Select Case nMode
        Case lcEqual
...
Public Property Get SelCount() As Long
    ...
    For lCount = LBound(mItems) To mItemCount
        If GetFlag(lCount, lcChecked) Then
    ...
    With picList
        ...
        If .Visible Then
            ...
            If Not mEditable Then
                UpdateEditText mListIndex, True
            End If
            
            If (mStyle = lcStandard) And (mListIndex >= 0) Then
                RaiseEvent Click
            End If
        ...
        ElseIf ListCount() > 0 Then
            ...
            If bDrawButton And (mBorderStyle <> lcBorderCustom) Then
                With UserControl
            ...
            If ListCount() > mDropDownItemsVisible Then
                If lColumnsWidth > lWidth Then
                    lHeight = (mR.TextHeight * mDropDownItemsVisible) + 2 + SCROLLBAR_SIZE
                Else
                    lHeight = (mR.TextHeight * mDropDownItemsVisible) + 2
                End If
            Else
                lHeight = (mR.TextHeight * ListCount()) + 2
            End If
            ...
            If mEditable Then
                mListIndex = SearchCode(UCase$(mSelectedText), lcEqual)
            End If
            mHighlighted = mListIndex
...
Private Sub SetFlag(ByVal nIndex As Long, nFlag As lcFlagsEnum, bValue As Boolean)
    Dim lCount As Long
    
    'Sets information by bit flags for a ListItem.
    
    If (nFlag = lcChecked) And mRequireCheckedItem Then
    ...
    If bValue Then
        If nFlag = lcChecked And (mStyle <> lcCheckboxes) Then
...
Private Sub SetFlags(nFlag As lcFlagsEnum, bValue As Boolean)
    Dim lCount As Long
...
Public Sub SetItem(ByVal vData As Variant, Optional ByVal DefaultIndex As Long = -1)
    Dim lCount As Long
    ...
    If Not bFound And DefaultIndex >= 0 Then
...
Private Sub SetListIndex(NewValue As Long)
    ...
    If mListIndex >= 0 Then
        mSelectedText = mItems(mRowPtr(mListIndex)).sValue(0)
        DrawEditText mInFocus
    Else
        mSelectedText = ""
        DrawEditText mInFocus
...
Private Sub SetScrollBars()
    ...
    nDropDownItemsVisible = (lHeight / mR.TextHeight)
...
Public Sub Sort(Optional Sort As Long = -1, Optional SortType As lcSortTypeEnum = -1, Optional SubSort As Long = -1, Optional SubSortType As lcSortTypeEnum = -1)
    '#############################################################################################################################
    ...
    With mCols(mSortColumn)
        If SortType = -1 Then
            If .nSortOrder = lcSTAscending Then
                .nSortOrder = lcSTDescending
            Else
                .nSortOrder = lcSTAscending
            End If
        Else
            .nSortOrder = SortType
        End If
...
Private Sub SortArrayCustom(ByVal lFirst As Long, ByVal lLast As Long, lSortColumn As Long, ByVal nSortType As Integer)
    ...
    For lIndex = lFirst + 1 To lLast
        bSwap = False
        If nSortType = 0 Then
            RaiseEvent CustomSort(True, lSortColumn, mItems(mRowPtr(lIndex)).sValue(lSortColumn), mItems(mRowPtr(lFirst)).sValue(lSortColumn), bSwap)
        Else
            RaiseEvent CustomSort(False, lSortColumn, mItems(mRowPtr(lIndex)).sValue(lSortColumn), mItems(mRowPtr(lFirst)).sValue(lSortColumn), bSwap)
        End If
...
Private Sub SortArrayDate(ByVal lFirst As Long, ByVal lLast As Long, lSortColumn As Long, ByVal nSortType As Integer)
    ...
    For lIndex = lFirst + 1 To lLast
        bSwap = False
        If nSortType = 0 Then
            bSwap = CDate(mItems(mRowPtr(lIndex)).sValue(lSortColumn)) > CDate(mItems(mRowPtr(lFirst)).sValue(lSortColumn))
        Else
            bSwap = CDate(mItems(mRowPtr(lIndex)).sValue(lSortColumn)) < CDate(mItems(mRowPtr(lFirst)).sValue(lSortColumn))
        End If
...
Private Sub SortArrayNumeric(ByVal lFirst As Long, ByVal lLast As Long, lSortColumn As Long, ByVal nSortType As Integer)
    ...
    For lIndex = lFirst + 1 To lLast
        bSwap = False
        If nSortType = 0 Then
            bSwap = Val(mItems(mRowPtr(lIndex)).sValue(lSortColumn)) > Val(mItems(mRowPtr(lFirst)).sValue(lSortColumn))
        Else
            bSwap = Val(mItems(mRowPtr(lIndex)).sValue(lSortColumn)) < Val(mItems(mRowPtr(lFirst)).sValue(lSortColumn))
        End If
...
Private Sub SortArrayString(ByVal lFirst As Long, ByVal lLast As Long, lSortColumn As Long, ByVal nSortType As Integer)
    ...
    For lIndex = lFirst + 1 To lLast
        bSwap = False
        If nSortType = 0 Then
            bSwap = mItems(mRowPtr(lIndex)).sValue(lSortColumn) > mItems(mRowPtr(lFirst)).sValue(lSortColumn)
        Else
            bSwap = mItems(mRowPtr(lIndex)).sValue(lSortColumn) < mItems(mRowPtr(lFirst)).sValue(lSortColumn)
        End If
...
Private Sub SortSubList()
    ...
    If mSortSubColumn > NULL_RESULT Then
        ...
        For lCount = LBound(mItems) To mItemCount
            bDifferent = mItems(mRowPtr(lCount)).sValue(mSortColumn) <> sMajorSort
            If bDifferent Or lCount = mItemCount Then
                ...
                sMajorSort = mItems(mRowPtr(lCount)).sValue(mSortColumn)
            End If
        Next lCount
    End If
End Sub

Public Property Get Style() As lcStyleEnum
Attribute Style.VB_ProcData.VB_Invoke_Property = ";Appearance"
    Style = mStyle
End Property

Public Property Let Style(ByVal NewValue As lcStyleEnum)
    mStyle = NewValue
    
    If mStyle = lcOptionButtons Then
        SetFlags lcChecked, False
    End If
    UpdateEditText mListIndex, mInFocus
...
'Added by Masino Sinaga (admin@masinosinaga.com), January 4, 2009
'This property for binding another controls to combobox in order
'to get value from a certain column in the drop-down list based
'on the selected item
Public Property Get MyControls() As MyControls
  Set MyControls = m_MyControls
End Property

Public Property Let MyControls(ByVal vNewValue As MyControls)
  Set m_MyControls = vNewValue
  PropertyChanged "MyControls"
End Property

'Added by Masino Sinaga (admin@masinosinaga.com), January 6, 2009
'This function for reset all value in combobox including
'the value in controls related to this combobox that using
'MyControls.Add...
Public Function ResetValue()
  Dim cont As Integer
  txtEdit.Text = mTextSelection
  For cont = 1 To m_MyControls.Count
    m_MyControls(cont).NameControl = ""
  Next cont
End Function
...
Public Property Let Text(ByVal NewValue As String)
    ...
    If mEditable Then
        If mStyle = lcStandard Then
            ...
            For lCount = LBound(mItems) To mItemCount
                If mItems(mRowPtr(lCount)).sValue(mSearchColumn) = NewValue Then
                    mListIndex = lCount
                    Exit For
                End If
            Next lCount
        ...
        DrawEditText mInFocus
    Else
        Err.Raise 383, "LynxComboBox", "Text is Read-Only"
    End If
    
    'PropertyChanged "Text"
    'Added by Masino Sinaga (admin@masinosinaga.com), January 6, 2009
    'for binding the value in certain column to another control
    'See the example of this in Sub LoadMultiColumnComboDB in frmMain
    'This following code would be fired when the text in txtEdit has been changed
    Dim cont As Integer
    For cont = 1 To m_MyControls.Count
      m_MyControls(cont).NameControl = ItemText(mListIndex, m_MyControls(cont).ColumnReturn)     'mItems(mItemCount).sValue(m_MyControls(cont).ColumnReturn)
    Next cont
...
Public Property Let ThemeStyle(NewValue As lcThemeStyleEnum)
...
Public Property Get ThemeStyle() As lcThemeStyleEnum
Attribute ThemeStyle.VB_ProcData.VB_Invoke_Property = ";Appearance"
    ...
    If Not mLockTextBoxEvent Then
        If mAutoComplete Then
            With txtEdit
                ...
                If Len(sText) > 0 Then
                    'lResult = SearchCode(UCase$(sText), lcLike)
                    lResult = SearchCode(UCase$(sText), lcGreaterEqual)
                    'RaiseEvent AutoCompleteSearch(lResult)
                    
                    If (lResult > NULL_RESULT) Then
                        mLockTextBoxEvent = True
                        .SelText = Mid$(mItems(mRowPtr(lResult)).sValue(0), lSelStart + 1)
                        .SelStart = lSelStart
        ...
        Dim cont As Integer
        For cont = 1 To m_MyControls.Count
          m_MyControls(cont).NameControl = ItemText(lResult, m_MyControls(cont).ColumnReturn)      'mItems(mItemCount).sValue(m_MyControls(cont).ColumnReturn)
        Next cont
...
Private Sub txtEdit_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If mAutoComplete And (Len(txtEdit.Text) > 0) Then
        Select Case KeyCode
        ...
        Case vbKeyReturn
            lResult = SearchCode(UCase$(txtEdit.Text), lcEqual)
            RaiseEvent AutoCompleteSearch(lResult)
...
Private Sub txtEdit_Validate(Cancel As Boolean)
    If mEditable Then
        If (mSelectedText <> txtEdit.Text) And (mListIndex >= NULL_RESULT) Then
            RaiseEvent EditTextChanged(mListIndex, mSelectedText, txtEdit.Text, Cancel)
        End If
                
        If Not Cancel Then
            mSelectedText = txtEdit.Text
        End If
    End If
End Sub

Private Sub UpdateEditText(lItemIndex As Long, bFocus As Boolean)
    Dim lCount As Long
    ...
    If mStyle = lcStandard Then
        If lItemIndex >= 0 Then
            mSelectedText = mItems(mRowPtr(lItemIndex)).sValue(0)
            DrawEditText bFocus
        End If
    Else
    ...
    Dim cont As Integer
    For cont = 1 To m_MyControls.Count
      m_MyControls(cont).NameControl = ItemText(lItemIndex, m_MyControls(cont).ColumnReturn)     'mItems(mItemCount).sValue(m_MyControls(cont).ColumnReturn)
    Next cont
...
Private Sub UserControl_InitProperties()
    ...
    mHotArrowColor = DEF_ARROWCOLOR
    mHotBorderColor = DEF_BORDERCOLOR
    mHotButtonBackColor = DEF_BUTTONBACKCOLOR
    ...
    If mEnabled Then
        If Shift And vbCtrlMask Then
            ...
            If KeyCode = vbKeyA Then
                ...
                SetFlags lcChecked, (SelCount() <> ListCount())
                Refresh
                RaiseEvent SelectionChanged
            End If
        End If
        
        If mLocked Then  'Added by Masino Sinaga (admin@masinosinaga.com), January 4, 2009
                         'in order if the Locked property value = True, then the item
                         'could not be selected in the drop-down list by
                         'using keypress from keyboard (bug-fixed)
        Else 'Added by Masino Sinaga (admin@masinosinaga.com), January 4, 2009
          If picList.Visible Then
              Select Case KeyCode
              Case vbKeyF4
              ...
              Case vbKeyUp
                  ...
                  ElseIf NavigateUp() Then
                      KeyCode = 0
                      UpdateEditText mHighlighted, False
                  End If
              Case vbKeyDown
                  If Shift And vbAltMask Then
                      mListIndex = mHighlighted
              ...
              Case vbKeySpace
                  If mHighlighted >= 0 Then
                      ...
                      SetFlag mHighlighted, lcChecked, Not GetFlag(mHighlighted, lcChecked)
                      DrawList
        ...
        End If  'Added by Masino Sinaga, Januari 4, 2009 (see its If above ... )
...
Private Sub UserControl_KeyPress(KeyAscii As Integer)
    ...
    If picList.Visible And Not mIgnoreKeyPress Then
        ...
        lResult = SearchCode(UCase$(sCode), lcGreaterEqual)
        If lResult > NULL_RESULT Then
            mListIndex = lResult
...
Private Sub UserControl_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If mEnabled And (mBorderStyle <> lcBorderCustom) Then
        With UserControl
            Call DrawEdge(.hdc, mButtonRect, EDGE_RAISED, BF_RECT)
            .Picture = .Image
        End With
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    mArrowColor = PropBag.ReadProperty("ArrowColor", DEF_ARROWCOLOR)
    ...
    mHotArrowColor = PropBag.ReadProperty("HotArrowColor", DEF_ARROWCOLOR)
    ...
    'm_SQLFields = PropBag.ReadProperty("SQLFields", "")
    '#############################################################################################################################    'Format Controls
    With txtEdit
        .Alignment = mAlignment
        .BackColor = mBackColor
...
'  Call PropBag.WriteProperty("SQLFields", m_SQLFields, "")
'  Call PropBag.WriteProperty("SQLTables", m_SQLTables, "")
'  Call PropBag.WriteProperty("SQLWhereCondition", m_SQLWhereCondition, "")
'  Call PropBag.WriteProperty("SQLSortField", m_SQLSortField, "")
'  Call PropBag.WriteProperty("SQLSortOrder", m_SQLSortOrder, "")
    ...
    '################################################################################
    'Appearance Properties
    Call PropBag.WriteProperty("ArrowColor", mArrowColor, DEF_ARROWCOLOR)
    ...
    Call PropBag.WriteProperty("HotArrowColor", mHotArrowColor, DEF_ARROWCOLOR)
